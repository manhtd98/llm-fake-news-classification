...
from core.vnabc.punctuation import Punctuation
...
class BasePhonemizer(abc.ABC):
    ...
    def _phonemize_postprocess(self, phonemized, punctuations, separate="|") -> str:
        """Postprocess the raw phonemized output

        Override this if you need a different behaviour
        """
        if self._keep_puncs:
            return self._punctuator.restore(phonemized, punctuations, separate)[0]
        return phonemized[0]
    ...
    def phonemize(self, text: str, separator="|", language: str = None) -> str:  # pylint: disable=unused-argument
        ...
        for t in text:
            ...
            phonemized.append(p)
        phonemized = self._phonemize_postprocess(phonemized, punctuations, separator)
        return phonemized
